...
def get_log_dir():
    ...
    return os.path.join('runs', f'training_{timestamp}')

# Create writer
writer = SummaryWriter(get_log_dir())
...
def train(model = None, save_path = '', config={},  train_dataloader=None, val_dataloader=None, feature_map={}, test_dataloader=None, test_dataset=None, dataset_name='swat', train_dataset=None):
    ...
    for i_epoch in range(epoch):
        ...
        # Log both losses
        writer.add_scalar('Loss/train', loss.item(), i_epoch)
        writer.add_scalar('Loss/acu', acu_loss, i_epoch)
        
        # Add learning rate logging
        writer.add_scalar('Learning_rate', optimizer.param_groups[0]['lr'], i_epoch)
